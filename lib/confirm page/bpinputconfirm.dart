...
import 'package:powerliftingapp/result%20page/outputbp.dart';
import 'package:video_player/video_player.dart';

class BPInputConfirm extends StatefulWidget {
  
  final String videoPath;

  const BPInputConfirm({super.key, required this.videoPath});

  @override
  State<BPInputConfirm> createState() => _BPInputConfirmState();
}

class _BPInputConfirmState extends State<BPInputConfirm> {
      ...
      if (response.statusCode == 200) {
        ...
        if (data.containsKey('processed_video') && data['processed_video'] != null) {
          ...
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => BenchpressOutput(videoBytes: processedVideoBytes),
            ),
          );
        }else {
