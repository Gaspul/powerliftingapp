...
import 'package:powerliftingapp/result%20page/outputdl.dart';

class DLInputConfirm extends StatefulWidget {

  final String videoPath;

  const DLInputConfirm({super.key, required this.videoPath});

  @override
  State<DLInputConfirm> createState() => _DLInputConfirmState();
}

class _DLInputConfirmState extends State<DLInputConfirm> {
  ...
  void _processFrame(File frameFile) async {
    ...
    var timeoutDuration = Duration(seconds: 1000);
      ...
      if (response.statusCode == 200) {
        ...
        if (data.containsKey('processed_video') && data['processed_video'] != null) {
          ...
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => DeadliftOutput(videoBytes: processedVideoBytes),
            ),
          );
        }else {
